...
- **An audit trail with a documented, readable, tamper-evident mechanism** — admin writes (`app/core/audit.py`) and per-request guardrail decisions (`app/core/guardrail_events.py`) are both hash-chained, append-only JSONL with a stated retention policy (see [Tamper-evident audit log](README.md#tamper-evident-audit-log)), verifiable offline against the log file itself, not an "immutable audit log" whose actual mechanism isn't published anywhere a buyer can check.
